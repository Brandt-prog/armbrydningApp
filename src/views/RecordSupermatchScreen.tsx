    ...
    <ScrollView style={styles.container}>
      ...
      <View style={styles.pickerRow}>
        {users.map((u) => (
          <Pressable
            key={u.id}
            style={[styles.pickChip, playerA?.id === u.id && styles.pickChipSelected]}
            onPress={() => setPlayerA(u)}
          >
      ...
      <View style={styles.pickerRow}>
        {users
          .filter((u) => u.id !== playerA?.id)
          .map((u) => (
            <Pressable
              key={u.id}
              style={[styles.pickChip, playerB?.id === u.id && styles.pickChipSelected]}
              onPress={() => setPlayerB(u)}
            >
      ...
      {playerA && playerB && (
        <>
          ...
          <Text style={styles.score}>
            {playerA.name} {games.filter((g) => g === 'A').length} — {games.filter((g) => g === 'B').length}{' '}
            {playerB.name}
          </Text>
          ...
          <Pressable
            style={[styles.button, (submitting || games.length === 0) && styles.buttonDisabled]}
            onPress={handleSubmit}
            disabled={submitting || games.length === 0}
          >
            <Text style={styles.buttonText}>
              {submitting ? 'REGISTRERER...' : 'AFSLUT OG REGISTRÉR SUPERMATCH'}
            </Text>
          </Pressable>
        </>
      )}
